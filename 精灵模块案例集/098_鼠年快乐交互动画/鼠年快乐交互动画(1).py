...
root = screen._root
screen.title('学好编程利国利民 ')

frames = ['res/bee.png','res/bee.png']
bee = Sprite(4)
bee.saycolor('red')
bee.saybordercolor('yellow')
bee.left(90)
bee.play('步步高古筝.wav')
def alt_costume():
    bee.nextcostume()    
...
def runcode():
    ...
    bee.left(45)
    bee.glide((-100,180),200)
    bee_group = Group('bee')
    for char in hengimage:
        Sprite(shape=char,pos=bee.pos(),tag='bee')  
        bee.addx(30)
        bee.right(15)
        bee.wait(0.1)
    left = '学编程利国利民'    
    ...
    bee.color('orange')
    bee.slide((0,-230),1000)
    bee.write('到啦。\n基于Python海龟画图的Python创意程序\nsprites版',align='center',font=('微软雅黑',12,'normal'))
    bee.setheading(90)
    bee.slide((0,0),3000)
    bee.saycolor('yellow')
    bee.saybordercolor((166,166,166))
    bee.say("祝你鼠年快乐",1000,False)
    bee.wait(2)
    while True:
...
b1 = TK.Button(root.frame_right,text='祝大家快乐',command=lambda:bee.say('祝大家快乐'))
b1.grid(row=0,column=0)

b2 = TK.Button(root.frame_right,text='来年平安,幸福',command=lambda:bee.say('来年平安,幸福'))
b2.grid(row=1,column=0)

b3 = TK.Button(root.frame_right,text='左转',command=lambda:bee.left(30))
b3.grid(row=2,column=0)

b4 = TK.Button(root.frame_right,text='右转',command=lambda:bee.right(30))
