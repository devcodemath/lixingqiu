...
def runcode():
    ...
    mouse.write('新年大礼到。\n转发此视频,赠送基于Python海龟画图的\nsprites模块的100个Python创意程序',align='center',font=('微软雅黑',12,'normal'))
    mouse.setheading(90)
    mouse.slide((0,0),3000)
    mouse.saycolor('yellow')
    mouse.saybordercolor((166,166,166))
    mouse.say("祝鼠年快乐",1000,False)
    mouse.wait(2)
    while True:
...
b5 = TK.Button(root.frame_right,text='背景颜色',command=lambda:screen.bgcolor(random.choice(_colorlist)))
b5.grid(row=5,column=0)

b6 = TK.Button(root.frame_right,text='微信号 scratch8 (李兴球)',command=lambda:mouse.say('微信号 scratch8 (李兴球)'))
...
popup.add_command(label="转发此视频到朋友圈或微信群",command=lambda:showinfo('hi','这是真的'))
popup.add_command(label="凭截图,即可免费获取",command=lambda:showinfo('hi','这是真的'))
popup.add_command(label="100个Python创意程序",command=lambda:showinfo('hi','这是真的'))
