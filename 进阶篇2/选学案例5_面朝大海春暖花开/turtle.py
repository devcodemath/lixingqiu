# 本程序给屏幕对象增加了onmousemove方法。在使用之前需要把屏幕的delay设为0
# 即screen.delay(0)，否则会出现超过最大递归数的错误。
...
def __methodDict(cls, _dict):
    """helper function for Scrolled Canvas，递归地求出类的所有方法"""
    baseList = list(cls.__bases__)
    baseList.reverse()
    for _super in baseList:
        __methodDict(_super, _dict)
    for key, value in cls.__dict__.items():
        if type(value) == types.FunctionType:# 如果值是函数
            _dict[key] = value
...
class TurtleScreenBase(object):
    ...
    def _drawpoly(self, polyitem, coordlist, fill=None,
        ...
        if top:
            self.cv.tag_raise(polyitem)#升到最顶上 lxqmark
...
##        """may be implemented for some other graphics toolkit"""
    ...
    def _onmousemove(self, fun, add=None):
        """绑定fun函数在画布的鼠标移动事件上，fun函数要有两个参数以接收
        鼠标指针的x和y坐标。        
        """
        if fun is None:
...
class Shape(object):
    ...
    def __init__(self, type_, data=None):
        ...
        self._rawimagedata = data        # 记录原始图形数据，让它来旋转
        self._data = data
...
class TurtleScreen(TurtleScreenBase):
    ...
    def onmousemove(self, fun, add=None):
        """Bind fun to mouse-movde event on canvas.

        Arguments:
        fun -- a function with two arguments, the coordinates of the
               clicked point on the canvas.
        """
        self._onmousemove(fun,add)
...
##############################  stamp stuff  ###############################
    ...
    #下面是给海龟增加面向鼠标指针的方法。
    #海龟调用facemouse命令后会自动不断地朝向鼠标指针（李兴球@2019/2/25)
    #def facemouse(self):        
    #    def get_mouse_cursor(x,y): 
    #        angle = self.towards(x,y)
    #        self.setheading(angle)            
    #    self.screen.onmousemove(get_mouse_cursor)
